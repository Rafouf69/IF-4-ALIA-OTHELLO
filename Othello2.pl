...
run:-
	...
	((Mode =< 2, play_interactive_game(Mode,Level,pos(0,1,_))	
	 ...
	 Mode =:= 3, play_automatic_game(Level,Level2,pos(0,1,_)))
	;
...
*************************************************************************/%.
play_automatic_game(Level,Level2,pos(Grid1,Computer1,_)):-
	...
	print_grid(Grid2),
	%sleep(1.5),
	play_automatic_game(Level2, pos(Grid2,Computer2,_))	% alternate turn
	;
...
play_interactive_game(Mode,Level,pos(GridId,Player1,_)):-	
	...
	(%computer to move
	 ...
	 (not(end_of_game(_)),not(user_exited_game),nl,
	  ...
	  print_grid(NewId),
	  ((Mode =:= Player1,!, print_a_compliment,!, sleep(0)) ; (sleep(0))),
...
get_user_name(PlayerName):-
	...
	name(PlayerName,[65,117,116,111,109,97,116,101,95,80,108,97,121,101,114,95,49]).
	%NO NEED IN TEST MODE
...
get_board_dimension(N):-
	...
	repeat,
	name(N,[$$$BOARD_DIMENSION$$$]).
	/* NO NEED IN AUTOMATIC MODE */
	/*
	get_user_input(N),
...
print_game_results(Mode,FinalGrid,PlayerName):-
	...
	nl,write('Total O''s on board: '), write(MinCount),
	...
	(Mode =:= 3,!,open('output.txt',write,Out),
    write(Out,MaxCount),
	write(Out,"-"),
    write(Out,MinCount),
    close(Out) )
