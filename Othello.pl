...
staticval(pos(GridId,_,_),Val,Level):-
	...
	(Level =:= 3,!,
	 ...
	 stable_count_evaluation(GridId,StableVal,_,_),
	 Val is (0.05 * CountVal) + (0.25 * MobilityVal) + (0.35 * CornersVal) + (0.15 * XVal) + (0.1 * CVal) + (0.1 * StableVal)).
...
%corners are stable
% pieces at the edge of the grid are stable if all pieces next to them
% are stable
...
get_max_depth(Level,MaxDepth):-
	((Level =:= 0,!, MaxDepth = 1)    % random player
	;
...
get_game_level(L):-
	...
	write('0 = Random Player'),nl,
	write('1 = Beginner'),nl,
	write('2 = Intermediate'),nl,
	write('3 = Advanced'),nl,
	write('4 = Godmod'),nl,
	get_user_input(L),
	((integer(L), L >= 0,L =< 4,!)	% validate input
	 ;
